...
repeat:
    ...
    haskid = 0;
    struct proc_struct *tmp=find_proc(1);
	list_entry_t *list = &proc_list,  *le = list;
	while ((le = list_next(le)) != list) {
		proc = le2proc(le, list_link);
		if (proc != NULL && proc->parent->pid==current->pid) {
			 haskid = 1;

		    if (proc->state == PROC_ZOMBIE ) {
		    	cprintf("pid = %d, name = %s, status switch from runnable to zombie and resource had been freed\n", proc->pid, get_proc_name(proc));
...
do_exit(int error_code) {
	...
	cprintf("pid = %d, name = %s, status switch from runnable to zombie\n", current->pid, get_proc_name(current));

    current->state = PROC_ZOMBIE;
    //cprintf("+3+pid = %d, status = %d\n", current->pid, current->state);
	bool intr_flag;
    struct proc_struct *proc;
    local_intr_save(intr_flag);
    {
...
proc_init(void) {
    ...
    //cprintf("pid = %d, name = %s, status switch from uninit to runnable\n", initproc1->pid, initproc1->name);
    //cprintf("pid = %d, name = %s, status switch from uninit to runnable\n", initproc2->pid, initproc2->name);
    //cprintf("pid = %d, name = %s, status switch from uninit to runnable\n", initproc3->pid, initproc3->name);
